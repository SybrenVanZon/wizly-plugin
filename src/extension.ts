...
async function setupAngularRuntimeSettings() {
    ...
    const serviceContent = [
        ...
        "            ? state.themes.filter(t => this.themeMatchesHost(t, this.getCurrentHost()))",
        "            : state.themes;",
        ...
        "            const host = this.getCurrentHost();",
        "            const keys = new Set(state.themes.filter(t => this.themeMatchesHost(t, host)).map(t => this.getThemeSelectionKey(t)));",
        "            return keys.size > 1;",
        "        }",
        "        return false;",
        "    }",
        ...
        "    }",
        "",
        "    private normalizeThemeHost(raw: unknown): string[] | undefined {",
        "        const hosts = Array.isArray(raw) ? raw : [raw];",
        ...
        "        return (host ?? []).map(h => h.toLowerCase()).sort().join(',');",
        "    }",
        "",
        "    private themeMatchesHost(theme: WizlyTheme, host: string): boolean {",
        "        const target = host.trim().toLowerCase();",
        "        return (theme.host ?? []).some(h => h.toLowerCase() === target);",
        "    }",
        "",
        "    canUserSwitchMode() {",
        "        return true;",
        ...
        "            const host = this.normalizeThemeHost(t?.host);",
        ...
        "            const dedupeKey = `${this.hostKey(t.host)}::${t.href}`;",
        "            if (!deduped.has(dedupeKey)) {",
        "                deduped.set(dedupeKey, t);",
        "            }",
        "        }",
        ...
        "        return `${name}::${this.hostKey(theme.host)}`;",
        "    }",
        "",
        ...
        "            const host = this.getCurrentHost();",
        "            const hostThemes = themes.filter(t => this.themeMatchesHost(t, host));",
    ...
    const componentsDirAbs = path.join(wizlyDirAbs, 'components');
    ensureDir(componentsDirAbs);

    const themeSelectorAbs = path.join(componentsDirAbs, 'wizly-theme-selector.component.ts');
    ...
    const modeToggleAbs = path.join(componentsDirAbs, 'wizly-theme-mode-toggle.component.ts');
    const modeToggleContent = hasMaterial
        ? [
            ...
            "</button>`",
            "})",
            "export class WizlyThemeModeToggleComponent {",
            "    private readonly settings = inject(WizlySettingsService);",
