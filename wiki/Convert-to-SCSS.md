...
- Updates relevant `package.json` / Angular workspace configuration when Sass support is missing
- Creates `src/scss/`
- Uses `src/scss/main.scss` as the main entry point
- Moves existing global style content into the SCSS structure
- Updates component `styleUrl` / `styleUrls` references from `.css` to `.scss` where needed
- Moves `src/styles.css` or `src/styles.scss` content into `src/scss/main.scss` and removes the old `src/styles.*` file afterwards
