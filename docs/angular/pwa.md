...
- Only runs when the workspace looks like a PWA (requires `src/manifest.webmanifest` and `ngsw-config.json`)
- Reads icon sizes and file paths from `src/manifest.webmanifest` (`icons[].src` + `icons[].sizes`)
